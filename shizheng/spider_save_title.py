...
url='http://www.xa.gov.cn/ptl/def/def/index_1121_6774_ci_trid_2456645.html'
area = '济南'
category_id = '1'
category_pid = '11'
def get_resp():
...
def get_detail():
    ...
    for r in res:
        try:
            ...
            print 'add someting'
            add_in='http://www.jinan.gov.cn'
            resp=requests.get(add_in+r.split(';')[1])
        doc=etree.HTML(resp.content)
        text_f='济南市政府网'
        title=doc.xpath('//td[@class="titlewzy"]/text()')
        title = etree.tostring(title[0], xml_declaration=True, encoding='utf-8')
