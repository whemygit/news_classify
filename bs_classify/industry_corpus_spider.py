...
def title_spider(industy_code,pagesize):
    ...
    industry_list=detail.xpath('//dl[@class="horizontal"]/dd/span[1]/strong/text()')
    with open('title_corpus','a') as fw:
        for i,title in enumerate(title_list):
            try:
                print industy_code,i,title,industry_list[i]
                fw.write(industy_code+'\x01'+title+'\x01'+industry_list[i]+'\n')
            except:
                continue
