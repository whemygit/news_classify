...
def set_category_id():
    sql = '''select bid,title,text from business_opportunity_copy WHERE category_id_1 IS NULL;'''
    res = db.query(sql)
    for r in res:
        category_list=[]
        print r.get('title')
        ...
        for i,keywds in enumerate(keywds_dict.values()):
            for keywd in keywds.split(","):
                if keywd in r.get('text'):
                    ...
                    break
        print category_list
        category_list_str=",".join(i for i in category_list)
        print category_list_str
        if category_list!=[]:
            update_category_1_sql='''UPDATE business_opportunity_copy SET category_id_1="%s" WHERE bid="%s";'''%(category_list_str,r.get('bid'))
            db.execute(update_category_1_sql)
            update_category_2_sql = '''UPDATE business_opportunity_copy SET category_id_2="%s" WHERE bid="%s";''' % (category_list[0], r.get('bid'))
            db.execute(update_category_2_sql)
        else:
            update_category_1_sql = '''UPDATE business_opportunity_copy SET category_id_1=0 WHERE bid="%s";''' % r.get('bid')
            db.execute(update_category_1_sql)
            update_category_2_sql = '''UPDATE business_opportunity_copy SET category_id_2=0 WHERE bid="%s";''' % r.get('bid')
            db.execute(update_category_2_sql)
