...
def change_is_resp(newsid):
    change_sql = '''update _news_data_classify_huanqiu set is_resp=1 where newsid=%s''' % newsid
    res = db.execute(change_sql)
    print res


def get_info(city):
    # sql = '''select * from _news_data_classify_huanqiu where area="%s" AND news_date="%s" AND img_show is not NULL AND is_resp=0 ORDER BY news_date DESC''' % (
    # city, date_n)
    # sql='''select * from _news_data_classify_huanqiu where area area="北京市"AND img_show is not NULL ORDER BY news_date DESC LIMIT 1;'''
    sql = '''select * from _news_data_classify_huanqiu where news_date="%s" AND img_show is not NULL ORDER BY news_date DESC limit 1;''' % date_n
    res = db.query(sql)
    for r in res:
        d = dict()
        ...
        d['isTop'] = 0
        d['isEssential'] = 0
        d['typeId'] = r.get('category_pid')

        resp = requests_post_1(d)
        print r.get('area'), resp.content,r.get('title'),r.get('classify_tag')
