...
for i in json.loads(jsonstr):
    key_words_dict.update({i.get('pid_title'):''})
for i in json.loads(jsonstr):
    key_words_dict[i.get('pid_title')]+=','+i.get('key_words')
for i in key_words_dict:
    print i,key_words_dict[i]

with open('keywords.csv','w') as fw:
    fieldnames=['classify_pid_title','classify_key_words']
