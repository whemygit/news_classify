...
def requests_post(data):
    ...
    resp = requests.post('http://117.78.41.235:8080/cityparlor-web/cityparlor/cityparlor/top/news/save', data=data,headers=headers)
    return resp
...
def change_is_resp(newsid):
    change_sql = '''update _news_data_classify_language set is_resp=1 where newsid=%s''' % newsid
...
def get_info():
    ...
    for r in res:
        ...
        print resp.content,r.get('title'),r.get('category_id'),json.loads(resp.content).get('retObj')
        post_result=resp.content
        news_id = r.get('newsid')
        change_is_resp(news_id)
        # return post_result,d
